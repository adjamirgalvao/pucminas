...
module.exports = class CompraService {
  ...
  static async updateCompra(id, compra) {
    const session = await Mongoose.startSession();
    session.startTransaction();

    try {
      let registro;
      let compraAtual = await this.getComprabyId(id, session);
      if (compraAtual) {
        registro = await CompraModel.updateOne({ _id: id }, { ...compra }, { session });

        //Removendo os itensCompras
        for (let i in compraAtual.itensCompra) {
          await ItemCompraService.deleteItemCompra(compraAtual.itensCompra[i]._id, session);
        }

        //Adicionando novamente os itensCompras
        for (let i in compra.itensCompra) {
          const novoItemCompra = {
            id_produto: compra.itensCompra[i].id_produto,
            id_compra: compraAtual._id,
            quantidade: compra.itensCompra[i].quantidade,
            preco: compra.itensCompra[i].preco
          };
          await ItemCompraService.addItemCompra(novoItemCompra, session);
        }
      }
  ...
  static async getComprabyId(id, session) {
    let retorno = null
    try {
      const registro = await CompraModel.aggregate(umaCompraItensFornecedorInnerJoinconst(id)).session(session);
