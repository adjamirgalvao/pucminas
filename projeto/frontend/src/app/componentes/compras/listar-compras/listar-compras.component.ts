...
export class ListarComprasComponent implements OnInit, OnDestroy {
  ...
  displayedColumns = [{ def: 'data', showMobile: true}, { def: 'numero', showMobile: false}, { def: 'fornecedor', showMobile: false}, { def: 'total', showMobile: true}, { def: 'acoes', showMobile: true}];
  ...
  excluirCompra(compra: Compra) {
    ...
    this.compraService.excluir(compra).pipe(catchError(
      err => {
        ...
        this.adicionarAlerta({ tipo: 'danger', mensagem: `Erro ao excluir a compra "${compra.numero}"! Detalhes: ${err.error?.error}` });
        ...
        () => {
          ...
          this.adicionarAlerta({ tipo: 'success', mensagem: `A compra "${compra.numero}" foi excluída com sucesso!` });
        });
  }
